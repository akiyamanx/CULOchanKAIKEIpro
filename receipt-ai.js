...
async function analyzeReceiptWithGemini(imageData, apiKey) {
  ...
  if (isSplit) {
    ...
    prompt = 'この画像に写っているレシート/領収書を全て個別に識別して解析してください。\n\n'
      ...
      + '- 【boundsの重要ルール】各boundsはそのレシート1枚だけをタイトに囲む矩形にする。他のレシートが入らないよう正確に指定する。レシートの紙の端をできるだけ正確に囲む。複数レシートが重なっている場合は見えている部分だけを囲む\n'
      + '- JSONのみを出力し、説明文は不要\n\n';
  }
  ...
  prompt += '【出力形式】\n'
    ...
    + '      "total": 1200,\n'
    + '      "type": "shopping",\n'
    + '      "bounds": {"x": 35, "y": 3, "w": 32, "h": 92},\n'
    + '      "items": [{"name": "エルボ", "qty": 4, "price": 120}]\n'
    + '    },\n'
    + '    {\n'
    + '      "date": "2025-11-13",\n'
    + '      "store": "タイムズ代々木",\n'
