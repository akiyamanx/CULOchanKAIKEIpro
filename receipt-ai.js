...
async function analyzeReceiptWithGemini(imageData, apiKey) {
  ...
  if (isSplit) {
    ...
    prompt = 'この画像に写っているレシート/領収書を全て個別に識別して解析してください。\n\n'
      ...
      + '- JSONのみを出力し、説明文は不要\n\n'
      + '【boundsルール】\n'
      + '- boundsは画像座標系（左上原点）で%指定。レシートの紙だけをタイトに囲む\n'
      + '- x=左端からの距離%, y=上端からの距離%, w=幅%, h=高さ%\n'
      + '- 隣のレシートが入らないようにする。レシートが1枚でも必ず返す\n\n'
      + '【cornersルール（最重要・正確に）】\n'
      + '- cornersはレシートの「紙の端」の4点のピクセル座標（画像の左上が原点）\n'
      + '- この座標で紙を切り抜いて正面に回転させるため、正確さが非常に重要\n'
      + '- 形式: [[x1,y1],[x2,y2],[x3,y3],[x4,y4]]\n'
      + '- 順序: 紙の物理的な角を、画像座標系で左上→右上→右下→左下の順\n'
      + '- 重要: 「画像上の左上」であり、「レシートの文字の向き」ではない\n'
      + '- つまり画像を見たときに紙の角が左上にある点が1番目\n'
      + '- レシートが横向きでも斜めでも、紙の角のピクセル座標を正確に指定する\n'
      + '- 座標は画像のピクセル単位（整数）。背景ではなく紙の端を指定する\n'
